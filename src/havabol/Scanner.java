...
/**
 * Reads in the source file from the user, as well as the symbol table (which
 * will not be used in assignment 1). Opens the file and begins to parse the
 * given file. Sets all lines of the file into an array list. Will create two
 * tokens, because of how it will be utilized later on in the project.
 * <p>
 * Note: If an error is encountered, will throw an exception to main to kill the
 * program immediately.
 * <p>
...
public class Scanner {
    ...
    /**
     * Constructor for the class. Verifies that the source file is viable, and
     * creates two new tokens for future use. Calls advanceLine() to set the
     * first line.
     * <p>
    ...
    /**
     * Advances through the source code, resetting the column position, and
     * setting currentLine to the next line of the code. Once the file size is
     * met, will break to return EoF.
     * <p>
     */
     ...
     * file when necessary. Automatically skips whitespace. Will only reach the
     * end of the method if it is the end of the file.
     * <p>
                ...
                if (!nextTokStr.isEmpty()) {
                    ...
                    switch (nextTokStr) {
                        ...
                        case "F":
                            ...
                            return;
                        //array_unbound key word
                        case "unbound":
                            setClassification(Token.OPERAND, Token.ARRAY_UNBOUND, true);
                            break;
                        // Functions:
                        case "LENGTH":
                        case "MAXLENGTH":
     ...
     * <p>
     * For operands, it will check the first digit to see if it is a number or
     * not. If it isn't a digit, it must be an identifier. If it is, checks to
     * see if it is a valid integer or float.
     * <p>
     * Note: If the string literal is not valid, it will call the error method
     * which will display the error message, and throw an exception.
     * <p>
     * @param primClassif Primary classification for the token
     * @param subClassif Subclassification for the token. If the token does not
     * have a subclassification, it will be passed in a 0.
     * @param classifComplete Boolean showing whether or not the token needs the
     * subclass to be set.
    ...
    /**
     * Builds a new string token based off of the quote, single or double,
     * given. Will break and create the string only if it encounters the same
     * quote again, and the previous character is not an escape character.
     * <p>
     * Note: If the string literal is not valid, it will call the error method
     * which will display the error message, and throw an exception.
     * <p>
     * @param quote Quote to build the string on. Will either be a single or
     * double quote.
     * @param strStart Starting position of the string. Has been pre-incremented
     * to throw away the first quote.
     */
    ...
    /**
     * Grabs the next token from the file. If at the end of the current line,
     * will advance to the next line in the file. Sets the current token to the
     * next token, and returns a string to the current token.
     * <p>
     * @return Functionally returns name of the next token.
     * @throws Exception Throws an exception to main if found in one of the
     * called methods
     */
    ...
    /**
     * Utility to reset to the beginning of a loop. Sets source line to the
     * start of the while loop and creates two new tokens. Because advanceline()
     * goes off current line number, it will set itself to the proper tokens.
     *
    ...
    /**
     * Method that turns on or off the debugging setup for displaying tokens,
     * expressions, and assignments. Three debug statements can be turned on:
     * <p>
     *
     * - bShowToken: print the token information for the current token returned
     * by scan.getNext(); this is actually done in getNext()
     * <p>
     * - bShowExpr: Print the result of each expression evaluation for
     * expressions involving at least one operator
     * <p>
    ...
    /**
     * Exits program and gives meaningful information at where and why the
     * program ended.
     * </p>
     *
     * @param diagnosticTxt Text for the user to be able to debug program
     * @throws Exception Will show stack trace of where, when, and why the
     * program ended.
     */
